...
const unsigned int edgestop = 16;
const int p =7; 
...
//    monomial start;
//    start.vars = {{900,p-1},{901,p-1}};
//
//    polynomial actual_poly;
//    actual_poly[start] = 1;

    vect_mono start;
    start.vars = {std::make_pair(900,p-1),std::make_pair(901,p-1)};
    iter_polyn actual_poly = {std::make_pair(start,1)};
    bool sub_in_dodgs = false;
    const int con_numvars = numvars;
    for(unsigned int i = 0; i < 9; i++){
        polynomial toadd;
//        polynomial_vect completed_dodgson;
//        toadd.reserve(actual_poly.size() * 10);
...
//            for(auto & elt : mono_it->first.vars){
            for(auto elt = mono_it->first.vars.begin(); elt !=  mono_it->first.vars.end(); ++elt){
//                    std::cout << mono_it->first  << "lookup \n";
//
                to_mult.push_back(tabled_lookup(elt->first,elt->second,sub_in_dodgs));
            }

            if(sub_in_dodgs == false){
...
//                                    cf %= p;
//                                    (*temp_poly)[vect_mono_key(mon_ins)] += cf;
                                }
...
//                               for(auto& thing : intermediate_poly){
//                                    if(thing.second != 1){
//                                    }
//                                    else {
//                                    }
...
//                       for(auto& mono : *(intermediate_poly)){
//                           completed_dodgson[mono.first] += mono.second;
//                       }
...
//                        }
        ...
        actual_poly.clear();
//        actual_poly.reserve(toadd.size());
        if(!sub_in_dodgs){
...
//        std::cout << i << '\n';
        ...
        if(sub_in_dodgs){
            std::cout << "broken \n";
//            std::cout << toadd[monomial(tocheck)] << " " << toadd[monomial(tocheck)] % p  << '\n';
//            for(auto& mon : completed_dodgson){
//                std::cout << mon.second << '*' <<  mon.first << '\n';
//                std::cout << numvars << '\n';
//            }
    ...
    if(sub_in_dodgs){
        ...
        for(auto monom = actual_poly.begin(); monom != actual_poly.end(); ++monom){
            long long int inter = monom->second;
            for(auto id = monom->first.vars.begin(); id != monom->first.vars.end(); ++id){
                inter *= tabled_lookup_fin(id->first,id->second); 
    //            inter %= p;
            }
